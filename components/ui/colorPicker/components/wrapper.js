...
const Wrapper = (props) => {
  ...
  const { setModalIsOpen, element, color } = props
  return (
    <>
      <Tabs
      tabs={tabs}
      ...
      <AnimatePresence mode="wait">
        {selectedTab === 'Preset' && (
          <motion.div key="preset" initial={{ x:-20,opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} transition={{ duration: 0.4,type: 'spring', bounce: 0.2 }} >
            <PresetView setModalIsOpen={setModalIsOpen} colors={colors} selectedColor={selectedColor} setSelectedColor={setSelectedColor} element={element} currentColor={color} />
          </motion.div>
        )}
        {selectedTab === 'Custom' && (
          <motion.div key="custom" initial={{ x:20,opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 20, opacity: 0 }} transition={{ duration: 0.4,type: 'spring', bounce: 0.2 }} >
            <CustomView setModalIsOpen={setModalIsOpen}  selectedColor={selectedColor} setSelectedColor={setSelectedColor} colors={colors} element={element}/>
          </motion.div>
        )}
      </AnimatePresence>
    </>
