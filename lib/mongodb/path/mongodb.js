...
export const addToPathDB = async (command) => {
  try {
    const client = await clientPromise;
    const db = client.db("SVGPathGenerator");
    const newCommand = await db.collection("commands").insertOne({
      ...
      firstControl: command.firstControl?command.firstControl:null,
      secondControl: command.secondControl?command.secondControl:null
    });
